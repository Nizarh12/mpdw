...
# 1. Bangkitkan model MA(2) dengan $\theta1 = 0.4$ dan $\theta2 = 0.6$ sebanyak 300 data (Manual dan dengan arima.sim)
...
Berdasarkan plot AFC tersebut, terlihat bahwa plot ACF cuts off di lag ketiga
...
# 2. Bangkitkan model MA(2) dengan $\phi1 = 0.5$ dan $\phi2 = 0.2$ sebanyak 300 data (Manual dan dengan arima.sim)
...
cor(yt_ar, yt_1_ar)
```
Korelasi antara $Y_t$ dengan $Y_{t-1}$ dari hasil simulasi mendekati perhitungan teoritis yaitu $\rho_1=\phi^1=0.625$
...
# 3. Bangkitkan model ARMA(2,2) dengan $\theta1 = 0.4$ & $\theta2 = 0.6$ & $\phi1 = 0.5$ & $\phi2 = 0.2$ sebanyak 300 data (Manual dan dengan arima.sim)
